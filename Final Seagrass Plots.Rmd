...
point_plot1 <- ggplot(grass.cont) + 
  geom_point(aes(x = MDS1, y = MDS2, fill = as.numeric(Year)),pch=21, size = 2, stroke=0) + 
  theme_classic() + 
  labs(fill = "Year") + 
  ...
  theme(panel.border = element_rect(fill = NA), 
        legend.position = "bottom") + 
  geom_label(data = grass_year, aes(x = MDS1, y = MDS2, label = sp), colour = "black", label.size = NA, alpha = 0.3, size = 3, fontface="italic") + 
...
Halo<-avggrass%>%
  ...
  scale_x_continuous(breaks = 2009:2022, labels=yearlabels)
...
propsea<-avgsea%>%
  ...
  scale_fill_manual(values=c("H. stipulacea" ="#F0E442","T. testudinium"="#A36186","H. writii"="#E0AFCA","S. filiforme"="#CC79A7"),labels = c(
      "H. stipulacea" = expression(italic("H. stipulacea")),
      "T. testudinium" = expression(italic("T. testudinium")),
      "H. writii" = expression(italic("H. writii")),
      "S. filiforme" = expression(italic("S. filiforme"))
    ))+
...
#possible colors
"#D9A2DC"  "#E0AFCA"  "#CC79CA"  "#AC79CC"  "#CD7A8F"  "#C27E79"  "#E0AFCA" "#EBc9DC" "#A36186"  "#7A4964" "#D9A2DC" "#CC79C8"  "#A16084", "#774761" "#F692CA"
