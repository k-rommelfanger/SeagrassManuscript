...
Using tidyverse to look a the total number of transects each year in the historical data, also determining the average cover per year
...
Tseagrass<-read.csv("Data/Historical Seagrass Data.csv")
...
library(gridExtra)
PatchPlot1<-grid.arrange(Native, Halo, Total)

```


Look at average seagrass per year

```{r}
AVGseagrass<-read.csv("Data/Historical Seagrass Data.csv")

#there is a blank in 2015 Transect 11, quadrat 10, nothing is recorded for S. filliforme, we replaced it with a zero, likely non of this species observed in this quadrat
AVGseagrass[is.na(AVGseagrass)] <- 0 

#calculate the average percent cover of each seagrass for each year
YearAvg<-Tseagrass%>%group_by(Year)%>%
  summarise(Thalassia=mean(T.testudinum), 
            Halophila=mean(Halophilastipulacae), 
            Halodule=mean(H.wrightii), 
            Syringodium=mean(S.filiforme))%>%
...
#average native cover begining 2016, 
MeanHalArrive<-YearAvg%>%
  select(Year, nativecov)%>%
  filter(Year>=2016)


mean(MeanHalArrive$nativecov)
#1.341729

sd(MeanHalArrive$nativecov)
#0.9792331
...
logits
#combine plots using gridextra because r is being annoying
grid.arrange(halo, native, ncol=2)
